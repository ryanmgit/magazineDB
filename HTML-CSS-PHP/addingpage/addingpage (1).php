...
<html>
<head>
<title>add Subscriber or Magazine page</title>
</head>
<body>
...
<p>Review ID:
<input type="text" name="reviewid" ></p>
<p>Review Date :
...
if(isset($_POST['addsubscriber'])){
  ...
  $query="insert into magazine (subscriberfname, subscriberlname, subscriberid, Email, City, State) values (:subscriberfname, :subscriberlname, :subscriberid, :Email, :City, :State)";//:accountname,:email are placeholders for user input
...
if(isset($_POST['addmagazine'])){
  //the following 6 lines of code define and execute an sql statment
  $query="insert into magazine (magazinename, magazineid, type, City, State) values (:magazinename, :magazineid, :type, :City, :State)";//:accountname,:email are placeholders for user input
  $sql=$pdo->prepare($query);
  $sql->execute(
...
if(isset($_POST['addsubscription'])){
  //the following 6 lines of code define and execute an sql statment
  $query="insert into magazine (magazineid, subscriberid, subscriptiondate, subscriptionperiod) values (:magazineid, :subscriberid, :subscriptiondate, :subscriptionperiod)";//:accountname,:email are placeholders for user input
  $sql=$pdo->prepare($query);
  $sql->execute(
  array(":magazineid" => $_POST['magazineid'],//the placeholders are defined using array()
        ":subscriberid" => $_POST['subscriberid'],
...
if(isset($_POST['addreview'])){
  //the following 6 lines of code define and execute an sql statment
  $query="insert into magazine (magazineid, subscriberid, stars, reviewid, reviewdate) values (:magazineid, :subscriberid, :stars, :reviewid, :reviewdate)";//:accountname,:email are placeholders for user input
  $sql=$pdo->prepare($query);
  $sql->execute(
  array(":magazineid" => $_POST['magazineid'],//the placeholders are defined using array()
        ":subscriberid" => $_POST['subscriberid'],
